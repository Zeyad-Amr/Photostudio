...
def draw_contour(Image, numberOfPoints, x_coordinates, y_coordinates):
    ...
    for i in range(numberOfPoints):
        ...
        img = cv2.line(Image, (y_coordinates[i], x_coordinates[i]), (
            y_coordinates[next], x_coordinates[next]), (0, 255, 0), 2)
...
def contour_area(numberOfPoints, x_coordinates, y_coordinates):
    ...
    j = numberOfPoints - 1
    for i in range(numberOfPoints):
        area += (x_coordinates[j] + x_coordinates[i]) * \
...
def contour_perimeter(x_points, y_points, points_n):
    ...
    for i in range(points_n):
        ...
        distance = points_distance(
            x_points[i], y_points[i], x_points[next_point], y_points[next_point])
        ...
        for i in range(points_n):
            ...
            for j in range(neighbours_size):
                ...
                if (current_x[i] < sobel_energy.shape[0] and current_x[i] > 0 and current_y[i] > 0 and current_y[i] < sobel_energy.shape[1]):
                    ...
                    point_energy = sobel_energy[int(current_x[i]), int(
                        current_y[i])] * -1 * gamma + internal_energy(current_x, current_y, points_n, alpha, beta)
...
def internal_energy(x_points, y_points, points_n, alpha, beta):
    ...
    for i in range(points_n):
        ...
        if angle2 < 0:
            angle2 += 2 * math.pi
        curv_sum += abs(angle1 - angle2 - avg_angle) * \
            abs(angle1 - angle2 - avg_angle)
...
def getChainCode(contourX, contourY):
    ...
    for i in range(0, numOfPoints):
        chaincode += getCodeBetweenTwoPoints(contourX[i % numOfPoints], contourY[i %
                                             numOfPoints], contourX[(i+1) % numOfPoints], contourY[(i+1) % numOfPoints])
